...
const fetchAndStoreImage = async (latitude, longitude) => {
    try {
      ...
      const imageUrl = `https://maps.googleapis.com/maps/api/streetview?size=600x300&location=${latitude},${longitude}&key=${API_KEY}`;
...
conn.once("open", async () => {
  try {
    ...
    const distinctCoordinates = await CrimeRate.aggregate([
      { $group: { _id: "$location.coordinates" } } 
    ]).toArray();

    if (!distinctCoordinates || distinctCoordinates.length === 0) {
      console.error("No distinct coordinates found.");
      return;
    }

    console.log(`Found ${distinctCoordinates.length} distinct coordinates. Processing...`);

    for (const entry of distinctCoordinates) {
      const [longitude, latitude] = entry._id;
      const crimeRecords = await CrimeRate.find({ "location.coordinates": entry._id }).toArray();
      const crimeRecordIds = crimeRecords.map(record => record._id);

      console.log(`Processing Image for Coordinates: (${latitude}, ${longitude}) with ${crimeRecordIds.length} crime references.`);

      await fetchAndStoreImage(latitude, longitude, crimeRecordIds);
    }

    console.log("Completed processing distinct coordinates.");
  } catch (error) {
